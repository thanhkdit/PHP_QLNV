  ...
  <div class="content  col-10 col-sm-9 col-xl-10">
      <div class="filter">
        ...
        <table class="table">
          ...
          <tbody>
            <td>
              <input type="text" id="fine_employee" name="fine_employee" class="form-control" size=5>
            </td>
            <td>
              <select class="form-control col-sm-8" id="fine_department" name="fine_department">
                <option selected value="">All</option>
                <?php
                  $show = $fine->show('t_Department');
                  if($show){
            ...
            <td>
              <select class="form-control col-sm-8" id="fine_fine" name="fine_fine">
                <option selected value="">All</option>
                <?php
                  $show = $fine->show('t_Fine_detail');
                  if($show){
                    while($result = $show->fetch_assoc()){
                ?>
                <option value="<?php echo $result['Fine_id']?>"><?php echo $result['Fine_id']." - ".$result['Fine_reason']; ?></option>
                <?php
                    }
                  }
                 ?>
            ...
            <td>
              <select class="form-control col-sm-8" id="fine_year" name="fine_year">
                <option selected value="">All</option>
                <?php
                  $show = $fine->show('t_Salary_year');
                  if($show){
                    while($result = $show->fetch_assoc()){
                ?>
            ...
            <td>
              <select class="form-control col-sm-8" id="fine_month" name="fine_month">
                <option selected value="">All</option>
                <?php
                  for ($i=1; $i<=12; $i++){
                ?>
      ...
      <h3 class="text-center display-block">Fine List</h3>
      <?php
        if(isset($deleted)){
          echo $deleted;
        }
      ?>
      <table id="list-fineInfo" class="table table-striped">
      ...
      <nav aria-label="Page navigation example">
        <ul class="pagination justify-content-center" id="fine-pagenumber">
